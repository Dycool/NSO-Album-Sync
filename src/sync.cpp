...
std::string capture_timestamp_prefix(std::int64_t timestamp) {
    ...
    const auto seconds = timestamp > 10'000'000'000LL
        ? timestamp / 1000
        : timestamp;
...
ExistingAlbumIndex index_existing_album(const std::filesystem::path& root) {
...
void preserve_capture_timestamp(
    ...
    try {
        ...
        // File timestamps are cosmetic; a successful download should stay valid.
    }
}
...
std::string SyncEngine::resolve_game_folder(
    ...
    if (!std::filesystem::exists(album_directory)) {
    ...
    for (const auto& directory :
         std::filesystem::directory_iterator(album_directory)) {
...
SyncResult SyncEngine::sync() {
    ...
    auto existing = index_existing_album(root);
    const auto title_folders = learn_title_folders(media, existing);

    const bool root_is_album_directory =
        lower(root.filename().string()) == "album";
    ...
    for (const auto& item : media) {
        ...
        const auto destination =
            album_directory / game_folder / filename;
