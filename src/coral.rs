...
impl CoralClient {
    ...
    pub fn self_presence(&self, session_token: &str) -> anyhow::Result<NintendoPresence> {
        ...
        Ok(response
            .get("result")
            .map(NintendoPresence::from_coral_result)
            .unwrap_or_default())
    }

    pub fn get_web_service_token(
        &self,
        session_token: &str,
        game_service_id: u64,
        ...
        let generation = self
            ...
            .unwrap_or_else(|poisoned| poisoned.into_inner())
            .generation;
        let target_id = if game_service_id == 0 {
        ...
        Ok(self
            .get_web_service_token_uncached(
                session_token,
                target_id,
                &session_hash,
                generation,
    ...
    fn get_web_service_token_uncached(
        &self,
        session_token: &str,
        target_id: u64,
        session_hash: &str,
        ...
        let access_token = self.ensure_session(session_token)?;
        let (mut na_id, coral_user_id) = {
            let state = self
                .state
                .lock()
                .unwrap_or_else(|poisoned| poisoned.into_inner());
            (state.na_id.clone(), state.user_id.clone())
        };
        if na_id.is_empty() {
        ...
        let _request_guard = self
            ...
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner());
        let (attestation, used_worker) =
            self.preferred_attestation(2, &access_token, &na_id, &coral_user_id)?;
        let mut attempt =
            self.perform_web_service_token_request(target_id, &access_token, &attestation)?;
        let mut recovered_from_worker = false;
        if used_worker && is_f_rejection_payload(&attempt.1) {
            let fallback =
                self.limited_nxapi_attestation(2, &access_token, &na_id, &coral_user_id)?;
        ...
        if response.status() / 100 != 2 {
            ...
            let suffix = if decoded.is_empty() {
                String::new()
            } else {
                format!(": {decoded}")
            };
        ...
        if recovered_from_worker {
            self.disable_worker(2);
        }

        let expires_in = json_i64(result, "expiresIn", 10_800);
        let ttl = expires_in.saturating_sub(120).max(60) as u64;
        let expires_at = now_seconds().saturating_add(ttl);

        {
            ...
            if state.session_hash == session_hash {
                state.web_tokens.insert(
                    target_id,
                    CachedWebServiceToken {
                        token: token.clone(),
                        expires_at,
                    },
                );
    ...
    fn ensure_session(&self, session_token: &str) -> anyhow::Result<String> {
        ...
        let now = now_seconds();
        {
            let state = self
                ...
                .lock()
                .unwrap_or_else(|poisoned| poisoned.into_inner());
            if state.session_hash == wanted_hash
        ...
        if let Some(restored) = self.restore_persistent_session(&wanted_hash, now) {
            return Ok(restored);
        }
        ...
        let (attestation, used_worker) = self.preferred_attestation(
            1,
            nintendo_tokens.id_token(),
            profile.id(),
            "",
        )?;
        let mut attempt =
            self.perform_login_request(nintendo_tokens.id_token(), &profile, &attestation)?;
        let mut recovered_from_worker = false;
        if used_worker && is_f_rejection_payload(&attempt.1) {
            let fallback = self.limited_nxapi_attestation(
                1,
                nintendo_tokens.id_token(),
                profile.id(),
                "",
            ...
            attempt =
                self.perform_login_request(nintendo_tokens.id_token(), &profile, &fallback)?;
            recovered_from_worker = true;
        }
        let (response, decoded) = attempt;
        if response.status() / 100 != 2 {
        ...
        let user_id = result
            ...
            .and_then(|user| {
                user.get("id")
                    ...
                    .or_else(|| user.get("nsaId").and_then(value_as_id))
            })
            .unwrap_or_default();
        let expires_in = json_i64(credential, "expiresIn", 7_200).max(1) as u64;
        {
            ...
            anyhow::ensure!(
                ...
                "Coral authentication cancelled"
            );
            ...
            state
                .web_tokens
                .retain(|_, cached| now_seconds() < cached.expires_at);
        }
    ...
    fn coral_call(
        ...
        let response = self.http.post_bytes(
            &url,
            &encrypted,
            &[
        ...
        if response.status() / 100 != 2 {
            ...
            anyhow::bail!("Coral request failed (HTTP {}){}", response.status(), suffix);
        }
        let decoded = self.nxapi.decrypt_response(response.body())?;
    ...
    fn perform_login_request(
        &self,
        id_token: &str,
        profile: &crate::nintendo_auth::UserProfile,
        f: &FAttestation,
        ...
        let encrypted = self
            .nxapi
            .encrypt_request(&url, "", &serde_json::to_string(&body)?)?;
        let response = self.http.post_bytes(
            &url,
            &encrypted,
            &[
    ...
    fn perform_web_service_token_request(
        &self,
        service_id: u64,
        access_token: &str,
        f: &FAttestation,
        ...
        let encrypted = self
            .nxapi
            .encrypt_request(&url, access_token, &serde_json::to_string(&body)?)?;
        let response = self.http.post_bytes(
            &url,
            &encrypted,
            &[
    ...
    fn preferred_attestation(
        &self,
        method: i32,
        token: &str,
        na_id: &str,
        coral_user_id: &str,
    ) -> anyhow::Result<(FAttestation, bool)> {
        if self.worker_enabled(method)
            && let Ok(attestation) =
                self.worker_attestation(method, token, na_id, coral_user_id)
        {
            return Ok((attestation, true));
        }
        Ok((
            self.limited_nxapi_attestation(method, token, na_id, coral_user_id)?,
            false,
        ))
    }

    fn worker_attestation(
        &self,
        method: i32,
        token: &str,
        na_id: &str,
        ...
        let response = self.http.post_json(
            ...
            &[
                ...
                "User-Agent: nso-album-sync/2.0.0".to_owned(),
            ],
            30,
        )?;
        anyhow::ensure!(
        ...
        let result = FAttestation::from_parts(
            ...
            json.get("requestId")
                ...
                .or_else(|| json.get("request_id").and_then(Value::as_str))
                .unwrap_or_default()
                .to_owned(),
        ...
        anyhow::ensure!(
            !result.f().is_empty()
                ...
                && result.timestamp() != 0,
            "Native f-token response was incomplete"
        );
        Ok(result)
    }

    fn limited_nxapi_attestation(
        &self,
        method: i32,
        token: &str,
        na_id: &str,
        ...
        match method {
            1 => {
                ...
                while attempts
                    .front()
                    .is_some_and(|oldest| now.saturating_sub(*oldest) >= METHOD1_WINDOW)
                {
                    attempts.pop_front();
                }
                anyhow::ensure!(
                    attempts.len() < METHOD1_LIMIT,
            ...
            2 => {
                let attempts = limits
                    .method2
                    .entry(if coral_user_id.is_empty() {
                        fallback_key
                    } else {
                        coral_user_id.to_owned()
                    })
                    .or_default();
                while attempts
                    .front()
                    .is_some_and(|oldest| now.saturating_sub(*oldest) >= METHOD2_WINDOW)
                {
                    attempts.pop_front();
                }
                anyhow::ensure!(
                    attempts.len() < METHOD2_LIMIT,
        ...
        drop(limits);
        self.nxapi
            .generate_f(method, token, na_id, coral_user_id)
    }
    ...
    fn worker_enabled(&self, method: i32) -> bool {
        ...
        let now = now_seconds();
        match method {
            1 => now >= health.method1_disabled_until,
            2 => now >= health.method2_disabled_until,
            _ => false,
        }
    }
    ...
    fn disable_worker(&self, method: i32) {
        ...
        let mut health = self
            ...
            .unwrap_or_else(|poisoned| poisoned.into_inner());
        if method == 1 {
            health.method1_disabled_until = until;
        }
        if method == 2 {
            health.method2_disabled_until = until;
        }
    }
    ...
    fn restore_persistent_session(&self, wanted_hash: &str, now: u64) -> Option<String> {
        ...
        let json: Value = match serde_json::from_str(&stored) {
            Ok(value) => value,
            Err(_) => {
                let _ = SecureStore::erase(CORAL_CREDENTIAL_ACCOUNT);
                return None;
        ...
        if json_string(&json, "sessionHash") != wanted_hash {
            return None;
        }
    ...
    fn restore_web_service_tokens(&self, session_hash: &str) {
        ...
        let _ = SecureStore::erase(LEGACY_WEB_SERVICE_CREDENTIAL_ACCOUNT);
        let stored = SecureStore::get(WEB_SERVICE_CREDENTIALS_ACCOUNT)
            .ok()
            .flatten();
        let now = now_seconds();
                    ...
                    if json_string(&json, "sessionHash") == session_hash {
                        if let Some(tokens) = json.get("tokens").and_then(Value::as_object) {
                            for (id, record) in tokens {
                                let Ok(service_id) = id.parse::<u64>() else {
                                    continue;
                                };
        ...
        for (service_id, cached) in &state.web_tokens {
            if now < cached.expires_at {
                tokens.insert(
                    ...
                    json!({"accessToken": cached.token, "expiresAt": cached.expires_at}),
                );
            }
        }
...
fn is_f_rejection_payload(payload: &str) -> bool {
    ...
    matches!(status, 9403 | 9599)
        || matches!(message.as_str(), "Invalid token." | "Unexpected error.")
}
