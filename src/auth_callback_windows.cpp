...
constexpr wchar_t kUserSchemeKey[] = L"Software\\Classes\\npf71b963c1b7b6d119";
...
std::wstring read_default_value(HKEY root, const std::wstring& path) {
    ...
    if (RegQueryValueExW(key, nullptr, nullptr, &type, reinterpret_cast<BYTE*>(buffer.data()), &bytes) != ERROR_SUCCESS) {
        RegCloseKey(key);
        return {};
    }
    RegCloseKey(key);
...
bool write_string_value(HKEY key, const wchar_t* name, const std::wstring& value) {
...
bool current_handler_is_ours() {
    const auto command = read_default_value(HKEY_CLASSES_ROOT, std::wstring(kSchemeKey) + kCommandSuffix);
    const auto desired = desired_command();
    return !command.empty() && !desired.empty() && equal_case_insensitive(command, desired);
}

}  // namespace

bool register_nintendo_auth_protocol() {
    if (current_handler_is_ours()) return true;

    HKEY existing = nullptr;
    if (RegOpenKeyExW(HKEY_CLASSES_ROOT, kSchemeKey, 0, KEY_READ, &existing) == ERROR_SUCCESS) {
    ...
    HKEY scheme = nullptr;
    if (RegCreateKeyExW(HKEY_CURRENT_USER, kUserSchemeKey, 0, nullptr, 0, KEY_WRITE,
                        nullptr, &scheme, nullptr) != ERROR_SUCCESS) return false;

    const bool root_ok =
        write_string_value(scheme, nullptr, L"URL:NSO Album Sync Nintendo Account callback") &&
    ...
    const std::wstring command_path = std::wstring(kUserSchemeKey) + kCommandSuffix;
...
void unregister_nintendo_auth_protocol() {
    if (!g_created_handler || !current_handler_is_ours()) return;
    RegDeleteTreeW(HKEY_CURRENT_USER, kUserSchemeKey);
