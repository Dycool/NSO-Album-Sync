    ...
    pub fn load() -> anyhow::Result<Self> {
        ...
        match stored_token.as_str() {
            SECURE_MARKER | MAC_KEYCHAIN_MARKER | LINUX_SECRET_SERVICE_MARKER => {
                let token = SecureStore::get(SESSION_ACCOUNT).ok().flatten().unwrap_or_default();
                let nickname = config.user_nickname().to_owned();
                config.set_session(token, nickname);
            }
            "" | VOLATILE_MARKER => config.clear_session(),
            value if value.starts_with("dpapi:") => config.clear_session(),
            legacy_token => {
                let nickname = config.user_nickname().to_owned();
...
fn insert_string(output: &mut Map<String, Value>, root: &Value, current: &str, legacy: &str) {
    ...
    if !value.is_empty() { output.insert(current.to_owned(), Value::String(value)); }
}
...
fn insert_bool(output: &mut Map<String, Value>, root: &Value, current: &str, legacy: &str) {
    ...
    {
        output.insert(current.to_owned(), Value::Bool(bool_with_legacy_key(root, current, legacy, false)));
    }
}
...
fn string_with_legacy_key(root: &Value, current: &str, legacy: &str, fallback: &str) -> String {
    string_key(root, current).filter(|value| !value.is_empty()).or_else(|| string_key(root, legacy)).unwrap_or_else(|| fallback.to_owned())
}
fn bool_with_legacy_key(root: &Value, current: &str, legacy: &str, fallback: bool) -> bool {
    value_key(root, current).and_then(Value::as_bool).or_else(|| value_key(root, legacy).and_then(Value::as_bool)).unwrap_or(fallback)
}
...
fn display_last_sync(stored: &str) -> String {
    if stored.is_empty() { return "Never".to_owned(); }
...
pub fn config_directory() -> anyhow::Result<PathBuf> {
    ...
    {
        let base = std::env::var_os("APPDATA").ok_or_else(|| anyhow::anyhow!("APPDATA is unavailable"))?;
        Ok(PathBuf::from(base).join("NSOAlbumSync"))
    }
    #[cfg(target_os = "macos")]
    {
        let home = std::env::var_os("HOME").ok_or_else(|| anyhow::anyhow!("HOME is unavailable"))?;
        Ok(PathBuf::from(home).join("Library/Application Support/NSOAlbumSync"))
    }
    #[cfg(all(unix, not(target_os = "macos")))]
    {
        if let Some(base) = std::env::var_os("XDG_CONFIG_HOME") {
            ...
            let home = std::env::var_os("HOME").ok_or_else(|| anyhow::anyhow!("HOME is unavailable"))?;
            Ok(PathBuf::from(home).join(".config/NSOAlbumSync"))
        }
    }
